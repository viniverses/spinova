...
export const ProductCard = memo(
  ({
    ...
    const shouldQueryWishlist =
      !isWishlistItem && isWishlisted === undefined && showFavorite;
    const isWishlistedQuery = useIsWishlisted(product.id, {
      enabled: shouldQueryWishlist,
    });
    const isFavorite =
      isWishlistItem || (isWishlisted ?? isWishlistedQuery.data ?? false);
          ...
          {showFavorite ? (
            ...
            >
              <Ionicons
                ...
                size={20}
                className="-mt-[1]"
                color={isFavorite ? colors.primary.DEFAULT : "#FFFFFF"}
              />
            </Pressable>
          ) : null}
        </View>
